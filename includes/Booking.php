...
if ($methods == "insert" || $methods == "post"){
    insertDoctor($key,$doctor);
}
if ($methods == "update" ){
    $id = $_GET["id"];
    $val = $_GET["value"];
    updatebookings($key,$id,$val,$booking);
}
elseif ($methods == "selectall" || $methods == "getall"){
     $username = $_GET["username"];
    $password = $_GET["password"];
    $sql = "select id from users where email='".$username."' and password ='".$password."'";
    if(count($doctor->getAllDoctorBySql($sql)[0]) > 0){
        $_SESSION["user_id"] =  $doctor->getAllDoctorBySql($sql)[0]["id"];
        echo 1;
    }else{
        echo 0;
    }
    ...
    $id = $_GET["id"];
    if (!empty($id)) {
        echo json_encode($doctor->getDoctorById($id));
    }else{
        echo "Not there!";
    }
}
...
function insertUser($key,$doctor){
...
function updatebookings($key,$id,$val,$booking){
